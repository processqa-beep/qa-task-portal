...
const VERSIONS: VersionInfo[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      'Fixed deletion persistence so deleted task assignments and CEO impact entries stay permanently deleted across page refreshes.',
      'Created 5:00 PM IST automated Google Chat reminder (/api/cron/reminder-chat) targeting unsubmitted QA members on Monday through Friday (disabled on weekends).',
      'Configured 7:00 PM IST daily automated task summary card posting (/api/cron/auto-post-chat) for submitted daily task reports.',
      'Merged "Devlopment" and "Development" work types into single canonical "Development" category across forms, charts, and database API filters.',
      'Created dynamic /api/work-types endpoint to synchronize custom work type categories across all devices automatically.',
      'Updated notification timestamp parsing (formatDateTime) to render exact local creation times and relative timestamps.',
    ],
  },
  {
    version: 'v2.2.0',
    ...
    changes: [
      ...
      'Formatted Google Chat cards with colorful work type tags (e.g. 1. DEVELOPMENT: Task details...), bold member names in header title, and clean text layout.',
      'Added Task Delete option (Trash2 icon) with confirmation modal in the Task History table and mobile card views.',
      'Built dual-layer permanent persistence for Task Assignments so assigned tasks never disappear on page refresh.',
